...
import { getTokenApi } from '@/api/token/index'
import { getCookie, setCookie, deleteCookie } from './cookie'
import { getQueryString, delQueryString, queryParams } from './index'

const auth = new Auth(window.env.baseUrl, '')
const getToken = (resolve: (value: unknown) => void, reject: (reason?: any) => void, code: string) => {
  const params = {
...
// 登录逻辑
export function login(): void | Promise<unknown> {
